...
- Weekly counter tiles for 召喚結界 / 黑色坑洞 show remaining (剩 N / 7, mobile + desktop) with the fill still rising from the bottom with used progress; opt-in per-task `remaining` flag in `tracker.json` (display-only, no save migration)
