...
- Reminder bell keeps a visible hover on the hovered row (ring + text shift instead of a bg wash that matches the row) and its hint moves from the native bubble to the shared tooltip

## 2026-09-18 — 可見性分流與 JJ
...
### Chores
- Spike cleanup: temporary worker routes (`/spike-send`, `/fanout-test`, `/db-test`) plus the bearer helper and `SPIKE_SECRET` field deleted — fetch serves health + scheduled only; worker redeployed, env secret deleted
...
### Chores
- Server-push planning branch (`feat/server-push`): `docs/ledger/server-push.md` combines the Cloudflare setup checklist + step-by-step guide, six-alternative review, build order, and dated work log in one file (duplicate standalone plan file removed); worker named `mabiroutine-worker`; wrangler added as a devDependency (+ `workerd` build approval, `.wrangler/` ignored); `workers/mabiroutine-worker` scaffolded and deployed (health + bearer-guarded `/spike-send` + scheduled stub, 3 crons live, KV `PURPLE` bound, VAPID/secrets in worker env only); spike A lands **FCM 201**, adopting full-worker fanout (Vercel fanout route dropped) — record-size fix (RS 4000, FCM caps bodies under 4096); Brave joins the desktop test matrix with its push-messaging toggle note; `sw-push.js` gains a `push` display listener (server cards reuse the local-card contract: collapse tag, re-buzz, auto-dismiss, tap deep-link), reviews the recorded plan against alternatives (recommends a worker-side fanout spike, keeping Turso subs, adding a GH Actions backup trigger after primary proves itself, keeping Web Push/KV-dashboard decisions), and orders the build (spike → worker scaffold → Vercel routes → purple feed/watcher/admin)

### Features
- 深淵的黑色坑洞 tracker row (behind `?purple_hole=1`, branch-only): daily per-char counter ×3 on a 36h15m predicted cycle (anchor 2026-09-16 14:08 Taipei); shows only on spawn days per the 06:00 daily bucket, otherwise parks itself in 已隱藏項目 with a 非出沒日 note naming the next spawn (render-only, manual hide untouched, out of progress). The row badges the bucket's exact spawn time as two absolute-date badges (MM/DD HH:mm — no 昨日/明日, which lies across midnight for late-night players): dimmed struck-through 已過 for the past spawn, violet 下次 for what's coming; a fresh spawn shows one badge, a stale (>15 min past) spawn shows both. The calendar icon opens a read-only timetable popover (past 2 + next 3, 下次 highlight, 預測值 disclaimer, follows page scroll). The bell subscribes a 15-min-early notification on its own card tag (store v18→v19 `purpleHoleReminders`, local-only like the hourly lane, separate `?purple_hole=1` flag; DevTools `__mabiPurpleTick`/`__mabiPurpleFire`). Row desc now names the three spawn zones (女神庭園、冰霜峽谷、雲海曠野各生成一個). The card names no one — body is `女神庭園、冰霜峽谷、雲海曠野各生成一個，預計 XX 分鐘後出現。` with live minutes (15 on schedule, fewer on catch-up). Spawn-day visibility follows the 06:00 daily bucket (the row showing through 09-17 for the 09-18 02:23 spawn is intended heads-up). Maintenance pause math is in place but fed empty in phase 1. Review fixes: per-char hidden no longer silences other characters' cards, midnight formats as 00:xx (h23), timetable opens on off-days too, overlapping maintenance windows merge instead of double-counting, v19 re-prunes the hourly lane, popover uses region role so it can't veto SW reloads
...
- Reminder copy polish: bell tooltip is now 訂閱通知 / 取消訂閱通知, soft-ask is titled 訂閱通知：不祥的召喚結界 with a rewritten body and 取消 button, denied dialog body rewritten in plain words with a single 關閉 button and silent auto-retry
...
### Features
- Event reminder for 不祥的召喚結界 (MVP, local-only): tap the 🔔 on the task to get one collapsed system notification at :00 Taipei sharp, landing with the game's soft ping ~2.5 minutes before the verified XX:02:30 start — only while still undone, auto-dismissing, never stacked, silent when done or inside 30s of the start. Tapping the card deep-links to the task row: the active character is kept when it's undone, otherwise the first undone character is selected, then the row smooth-scrolls into view with one subtle amber flash (SW `notificationclick` in prod via `sw-push.js`, page `onclick` in dev). The bell soft-asks first (訂閱開場提醒？ — what it does, device-only, one-tap cancel, browser asks next) so the system permission prompt never fires cold; the confirm dialog gains neutral styling for non-destructive use The card names the characters, not the counts (`不祥的召喚結界出現了` / `A、B、C`, capped at 3 + 等N隻; hidden and finished characters omitted — no clock time, no lead text anywhere). Late openers get an immediate catch-up card. The bell sits next to the row title (mobile + desktop). Bell and scheduler are scoped to an eligibility list (today just `barrier`). Subscriptions stay on this device (never synced, never sent anywhere, store v17→v18) and only fire while the app is open; closed-app push is a follow-up. DevTools handles `__mabiHourlyTick()` (countdown readout) and `__mabiHourlyFire()` (force one card now)
...
- Reminder no longer re-fires every second inside the catch-up window: the post-fire re-arm reused the catch-up scheduler and looped until the cutoff — now it arms strictly next hour, plus an epoch-hour dedup so a foreground bounce right after a fire can't double-card
...
### Features
- Pinned barter follows the canonical file order (town → npc → shops) unless rearranged by drag — first drag snapshots a personal order, new pins slot in otherwise; existing arrangements reset once on upgrade (store v16→v17), progress untouched
...
### Fixes
- Local sync backend hardening: `busy_timeout` on the file DB (parallel PATCHes no longer take dev:api down with SQLITE_BUSY), the API handler answers 500 instead of throwing, and api-live inspects the local file DB with a localhost default base — the live gate now passes against `pnpm dev:api` for real
...
- One-time Redis→SQL session export (`scripts/migrate-upstash-to-sql.mjs`, docs/sql-migration.md P4): scans a namespace, copies hash sessions into `sessions`+`kv` and bare v1/v2 blobs into `sessions.legacy`, preserving updatedAt/seq/remaining TTL; dry-run by default, `--apply` writes, idempotent. Verified read-only against prod (5 hash + 7 legacy sessions, 1415 kv rows) and applied into a local file target
- Sync storage ported from Upstash Redis to SQL (`api/_db/*`, `api/session.ts`): one `sessions` probe row plus one `kv` row per flat key, PATCH written as one atomic unit, cycle-key nulls become physical deletes (persistent tombstones retained), local `file:` (node:sqlite) and remote Turso (`@libsql/client/web`) drivers behind one `Db` interface, a `_schema_version` migration runner, an in-memory rate limiter in place of the Redis `INCR`, and lazy 180-day TTL reclamation on read. New `scripts/sync-tests/sql-smoke.mjs` (wired into `pnpm test:sync`) covers the handler on a local file and, with `--remote`, the real Turso DB; `api-live` is backend-agnostic via `scripts/sync-tests/backend.mjs`. Cycle-key GC retention cut 60d→8d (`GC_DAYS`), with E6 reworked to pin the new boundary
- Migration plan + ledger for moving sync storage from Upstash Redis to SQLite (Turso free, Tokyo primary): `docs/sql-migration.md` records the lock on per-field LWW (`PK(session_id, key)`, wire key kept verbatim incl. `@bucket`), the GC rule that nulls of cycle keys are real deletes (persistent tombstones retained, so "live rows only" actually holds at GC_DAYS 8), one atomic batch per PATCH (kv rows + the `sessions` probe row), a DB-agnostic `Db` layer for SQLite/Postgres, the verified Turso/Upstash uptime and region research (prod functions already execute in Tokyo, confirmed via `X-Vercel-Id: hkg1::hnd1`), the corrected 1000-user quota estimate (N≈230 rows/session; reads ~48% at 1000 heavy 2-device users, so ~2x headroom; rows-written is the only ceiling, and rate limiting stays a simple in-memory per-instance cap, no WAF, no SQL counter), and a seamless migration path (bulk export of live sessions + a 7-day lazy read-through fallback, no re-link)
...
- Sync quota hardened for release: one 5min foreground loop (the App 60s interval is gone — two timers doubled every poll), unchanged polls cost 2 commands via a timestamp-only `?meta=1` probe that skips the full GET, and the 180-day TTL renews on a daily `touch` beacon instead of every request. The repoll pauses after 15min without input (any input/focus/visibility resumes with a full pull-then-reset round), and every device keeps a 14-day per-day request counter (`__mabiSyncStats()` in DevTools) so the budget is measured, not guessed — Q1–Q5 join the gate. Always-open active tab ~17K/mo, idle tab ~0; live gate covers meta/touch/legacy-flag paths (E2E caught and fixed one real adoption-starvation bug in the probe: post-push rounds always full-GET)
...
- Pinned tracker rows show the same materials without reflowing the checklist: hovering (desktop) or tapping (touch) the give text opens a lightweight card with the full sources inline — no modal, no focus trap, arrow pointing at the source, sized to fit (scrollbar only when taller than the viewport). Gather-preferred throughout: gatherable items show 自採 only, and panels that would render a lone 自採 are hidden entirely (no toggle)
- Tracker material display consolidated on the hover card: the prototype overlay-modal/line/drawer shells and the ?shell switcher are deleted — pinned rows always use the hover card now. (Corrects the previous entry, which described the hover card while prod still defaulted to the modal.)
- Hover card drops its inner gray panel: the breakdown content sits directly on the popover (the box only remains in explorer rows, where it separates breakdown from row)
- Hint tooltips rebuilt on Radix (`@radix-ui/react-tooltip`): the hand-rolled absolute bubble was clipped by the section Cards' `overflow-hidden` (the 釘選 badge hint) and never flipped at viewport edges — both call sites (tracker badge, explorer pin buttons) work unchanged, content now portals out and flips automatically
- Tracker hover card stops overflowing: three stacked causes fixed — the desktop row's `truncate` span leaked `white-space: nowrap` into the card (reset at the card root), one no-wrap flex row in variant B (now wraps), and a fixed 46dvh cap replaced by measure-then-fit sizing; card is 430px on desktop, compact on mobile
...
- Review follow-ups: town filter gets the same stale-value fallback, 已篩選 shows whenever any filter is engaged, `rest`→`other`, `SHORT_KIND` narrowed to `RouteKind`
- PWA goes silent-fresh: navigations are NetworkFirst (4s timeout → cache, preload hides the RTT), so every online load renders the current build — no more stale flash, forced reload, or 已更新到新版本 toast on every deploy. SW updates only refresh caches in the background (prompt mode acknowledged silently, reload suppressed); offline loads still fall back to cache. Hashed-asset CacheFirst and NPC-art SWR unchanged. One structural catch found by E2E and fixed: generateSW's precached NavigationRoute + precache directoryIndex both shadow a NetworkFirst navigate route (reload-after-deploy rendered the old rev), so there is no navigateFallback and index.html stays out of the precache
...
- First paint goes fresh sooner on synced launches: `index.html` fires the session GET before the module graph loads so the round trip overlaps JS bootstrap (consume-once, id-matched, 60s TTL, live-GET fallback; the acknowledged-push overlay covers the pre-flush read, and `/api/*` matches no SW route)
- Malformed request bodies can no longer kill the dev server: `bodyOf` catches the platform JSON-parse throw (vercel dev raises instead of answering 400) and treats the body as absent, so the existing validators answer 400/404; production behavior unchanged
...
- Weekly reset no longer keeps last week's state when the rev-3 upgrade lands after a rollover: provenance-less values are now checked against the last-reset markers, and a stale marker drops them instead of stamping them current (stamping laundered Sunday's checks into Monday's bucket, where no prune could ever remove them and sync spread them to every device); upgrades with current markers and marker-less saves behave exactly as before
- Unchecking a box no longer re-checks itself after the next sync: user clears now write explicit values (uncheck → `false`, counter-zero/clear-section → `0`) instead of deleting — deletion stays silent on the wire by design (resets must not propagate), so the server's old value resurrected on every pull; explicit falses travel the ordinary value path and converge, with no tombstones and no ping-pong
- Deleting a character no longer resurrects it on the next sync: merges drop characters with no live name key (the name tombstones on delete while value keys linger till GC); `resetAll` converges the same way, and editing a custom task's kind re-stamps provenance instead of letting the next prune eat the stored check
- 每週挑戰 is now account-shared (was per-character) and 野外首領尾刀 is removed: existing progress migrates to the shared slot (best across characters, capped at 9), hides follow, and the removed id is pruned everywhere (store v14); old per-character sync keys age out on their own — nothing to re-link
...
- Text fields no longer trigger iOS auto-zoom on focus: `Input`/`Textarea` render at 16px instead of 14px (viewport `maximum-scale` deliberately untouched so pinch-zoom keeps working)
