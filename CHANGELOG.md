...
### Features
- 深淵的黑色坑洞 tracker row (behind `?purple_hole=1`, branch-only): daily per-char counter ×3 on a 36h15m predicted cycle (anchor 2026-09-16 14:08 Taipei); shows only on spawn days per the 06:00 daily bucket, otherwise parks itself in 已隱藏項目 with a 非出沒日 note naming the next spawn (render-only, manual hide untouched, out of progress). The row badges the bucket's exact spawn time as two absolute-date badges (MM/DD HH:mm — no 昨日/明日, which lies across midnight for late-night players): dimmed struck-through 已過 for the past spawn, violet 下次 for what's coming; a fresh spawn shows one badge, a stale (>15 min past) spawn shows both. The calendar icon opens a read-only timetable popover (past 2 + next 3, 下次 highlight, 預測值 disclaimer, follows page scroll). The bell subscribes a 15-min-early notification on its own card tag (store v18→v19 `purpleHoleReminders`, local-only like the hourly lane, separate `?purple_hole=1` flag; DevTools `__mabiPurpleTick`/`__mabiPurpleFire`). Row desc now names the three spawn zones (女神庭園、冰霜峽谷、雲海曠野各生成一個). The card names no one — body is the predicted spawn time (`預計 MM/DD HH:mm 出沒`, copy still TBD). Spawn-day visibility follows the 06:00 daily bucket (the row showing through 09-17 for the 09-18 02:23 spawn is intended heads-up). Maintenance pause math is in place but fed empty in phase 1
