...
// R: v17 save with hourly reminder subscriptions -> v18 keeps live +
// custom ids, prunes removed rows; progress untouched
{
  assert(trackerIds.has("parttime"), "R premise: parttime exists (update fixture if removed)");
  const out = migratePersisted(
    {
      version: 17,
      characters: [{ id: "c1", name: "A", taskValues: { parttime: true }, hiddenTaskIds: [] }],
      activeCharId: "c1",
      customTasks: [{ id: "custom1" }],
      hourlyReminders: ["parttime", "custom1", "gone-xyz"],
    },
    17
  ) as AnyRec;
