...
- `src/data/barter.json` (93 rows: notebook 70 + yenyen union; 6 `yen-` dups/conflicts dropped 2026-09-06, 4 more `yen-` twins dropped 2026-09-09 with user state remapped to the kept twin, then +5 new rows: edern/erin×2/cat-sea-bass/jennifer) is hand-owned. Full seed is notebook 70 tw as base, extended to yenyen 86 after diff, never include notebook 18 kr.
- `src/data/recipes.json` (135 items, 2026-09-09) holds base-ingredient routes for every barter give: make (station/level/components/batch/skill), shop (NPC/town/limit), barter cost, gather/quest/drop leaves. Curated from yenyen `/items` (private-local scraper, raw stays in gitignored `suggestions/`) under the same TW-only rule: KR-flagged recipes ship only after in-game verification (`verified: tw-ingame`); known source quirks recorded inline (yenyen `棉` typo = `麵`, `完美繃帶` sourceless). Always write `鍊金`, never `煉金`. `鱸魚` carries two routes: gather-first 釣魚 Lv.25 plus the 貓商人 barter leg (喵幣×20000, 每週 1 次 — the pre-25 pipeline). `銀合金錠` carries four: 金屬加工設備 Lv.4 make (特殊鋼錠×5 + 銀礦石×20 + 煤炭×16, ×3/batch, 約 5 小時 in `note` — no `duration` field in the schema) plus the three non-situational barter legs (艾頓 每週×10, 阿爾米斯 每日 server-shared, 艾琳 每日); the situational 艾琳 合金鋼錠×30 leg is intentionally untracked (the 別換-tier filter would drop it anyway). `鱸魚辣魚湯` is a 食物製作台 Lv.4 make (料理 Lv.20, outQty 1 like all food entries); its 白蘿蔔/辣椒粉 have no entries yet and render 找不到資料. Gather-only leaves (銀礦石, 喵幣) intentionally carry no entries: 喵幣 only ever appears as a barter cost inside an alternative route so it never surfaces, while 銀礦石 is a real make component and shows 找不到資料 until it gets a 採礦 entry. → resolved: 銀礦石 now 採礦 Lv.25 (also cleans up the pre-existing 特殊鋼錠 line). 胡椒 already covered both 格莉娜 + 康納, no change. 白蘿蔔/辣椒粉 (珍妮佛 shop) still pending town + limits. → resolved: both 珍妮佛/班克爾 每週 30 次 (gold prices 10000/3000 kept in entry notes — shop `components` only models item tokens, never gold). Soup now expands with zero missing leaves. (珍妮佛 NPC portrait still to come.) `休馬斯` chain (2026-09-11): 沙威瑪 is a 食物製作台 Lv.4 make (料理 Lv.13, outQty 1 per food convention), 卓越繃帶 a 藥品製作台 Lv.4 make (藥水調製 Lv.22, ×5/batch — user wrote 藥物製作台, recorded as 藥品製作台 per station precedent); supporting barter legs derived from barter.json (高級布料+ via 潔米, 清淡的肉 via 珍妮佛, four 結晶 via 史帝華). Only remaining unknown leaf: 傷痕花粉末. → resolved: 藥品加工設備 Lv.4 (傷痕花×20 → ×5, ~25min in `note`); 傷痕花 itself still sourceless. → resolved: 採集藥草 Lv.20. Both chains now expand with zero unknown leaves.
