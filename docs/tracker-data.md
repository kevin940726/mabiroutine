...
- `src/data/barter.json` (94 rows: notebook 70 + yenyen union; 6 `yen-` dups/conflicts dropped 2026-09-06, 4 more `yen-` twins dropped 2026-09-09 with user state remapped to the kept twin, then +5 new rows: edern/erin×2/cat-sea-bass/jennifer) is hand-owned. Full seed is notebook 70 tw as base, extended to yenyen 86 after diff, never include notebook 18 kr.
- `src/data/shops.json` (2026-09-12, schema'd by `shops.schema.json`, `$schema`-pointed for editor validation/autocomplete) is the shopping DB: NPC → purchase/exchange options, blocks sorted by town for in-game checking (`kind` omit = shop, spell out barter) with `cost {amount, currency}` (currency omit = gold; gold = per single item, item tokens = per whole exchange; `amount: null` = unfilled gold, enforced token-non-null by the check), `get {amount}` received per exchange (omit when 1), `limit {times, period}` (omit when uncapped; omitted times = uncapped (a bare 1 must be written out); period defaults to daily when times is present, rejected without times), `scope: character|account` (omit = character; account covers the old 伺服器-suffixed caps, today exactly the perChar=false rows). `materials.ts` synthesizes every option into a route at load and merges it with the recipes make/gather/quest routes — single read path, no positional zip, no dual truth. `barter.json` stays the ranking/display source (priority/rec/pins/notes/perChar); exchange mechanics live here, checked-not-generated. Explorer cap text resolves through `twinTradeLeg` (exact npc+item+outQty shops match owns the display; barter string is fallback only). After touching any of the three files, run `pnpm test:shops` (strict shape + currency + dup + no-trade-routes-in-recipes + shop-gold-only + twin cap parity; in the `pnpm check` gate since 2026-09-12).
- `src/data/recipes.json` holds make routes (station/level/components/batch/skill) plus gather/quest/drop leaves — never shop/barter (102 legs stripped 2026-09-12 into `shops.json`, 37 emptied entries deleted; `完美繃帶` keeps its pre-empty `routes: []`). Curated from yenyen `/items` (private-local scraper, raw stays in gitignored `suggestions/`) under the same TW-only rule: KR-flagged recipes ship only after in-game verification (`verified: tw-ingame`); known source quirks recorded inline (yenyen `棉` typo = `麵`, `完美繃帶` sourceless). Always write `鍊金`, never `煉金`. `鱸魚` resolves two ways: gather-first 釣魚 Lv.25 plus the 貓商人 barter leg (喵幣×20000, 每週 10 次 — corrected 2026-09-12 after the twin-parity check caught barter.json saying 每週 1 次). `銀合金錠` carries four: 金屬加工設備 Lv.4 make (特殊鋼錠×5 + 銀礦石×20 + 煤炭×16, ×3/batch, 約 5 小時 in `note` — no `duration` field in the schema) plus the three non-situational barter legs (艾頓 每週×10, 阿爾米斯 每日 server-shared, 艾琳 每日); the situational 艾琳 合金鋼錠×30 leg also lives in `shops.json` now but the rank engine only falls back to it (once/situational tier). `鱸魚辣魚湯` is a 食物製作台 Lv.4 make (料理 Lv.20, outQty 1 like all food entries); its 白蘿蔔/辣椒粉 have no entries yet and render 找不到資料. Gather-only leaves (銀礦石, 喵幣) intentionally carry no entries: 喵幣 only ever appears as a barter cost inside an alternative route so it never surfaces, while 銀礦石 is a real make component and shows 找不到資料 until it gets a 採礦 entry. → resolved: 銀礦石 now 採礦 Lv.25 (also cleans up the pre-existing 特殊鋼錠 line). 胡椒 already covered both 格莉娜 + 康納, no change. 白蘿蔔/辣椒粉 (珍妮佛 shop) still pending town + limits. → resolved: both 珍妮佛/班克爾 每週 30 次 (gold prices 10000/3000 structured as `price` in `shops.json` and rendered — face tooltips show 金/個, L3 appends a 金幣×N total; the old entry notes are gone with the stripped entries; shop `components` only models item tokens, never gold — enforced by the check). Soup now expands with zero missing leaves. (珍妮佛 NPC portrait still to come.) `休馬斯` chain (2026-09-11): 沙威瑪 is a 食物製作台 Lv.4 make (料理 Lv.13, outQty 1 per food convention), 卓越繃帶 a 藥品製作台 Lv.4 make (藥水調製 Lv.22, ×5/batch — user wrote 藥物製作台, recorded as 藥品製作台 per station precedent); supporting barter legs derived from barter.json (高級布料+ via 潔米, 清淡的肉 via 珍妮佛, four 結晶 via 史帝華). Only remaining unknown leaf: 傷痕花粉末. → resolved: 藥品加工設備 Lv.4 (傷痕花×20 → ×5, ~25min in `note`); 傷痕花 itself still sourceless. → resolved: 採集藥草 Lv.20. Both chains now expand with zero unknown leaves.
...
- `perChar` / `limit` / `rec` / `region` come from notebook/yenyen, not invented. `perChar=false` rows (today 7: 麗莎×3, 阿爾米斯 銀合金錠, 康納 魔力石×2, 安黛莉 聖水×10) share one tracker value across every character (store v16, 伺服器 badge) — checking on any character checks all of them.
- File order is priority → `TOWN_ORDER` (`src/lib/towns.ts`, game-region order) → npc → `shops.json` items order within one npc (twin-less rows keep a stable tail). Same-priority ties in the rank engine resolve to the earliest row, so re-sorting can flip assumed plans — check the probe diff when re-sorting.
- **Default pins are a hand-owned list:** `src/data/defaultPins.json` (`pins` array of barter ids) — NOT derived from `priority==="must"`. Curate it by hand; when checking sources by hand, watch for must-priority drift the same way. Store sanitizes the list against barter.json ids at load (unknown ids dropped, order kept). Pinned rows with a `每週` limit render under the 每週以物易物 subsection (weekly bucket, Mon 06:00); either barter subsection hides when its cycle has no pins.
